...
We start by checking the ANOVA assumptions of normality and homogneity of variance.
...
null hypotesis of homoscedasticity. Additionally, we can't assume normality. For this
reason we will use the non-parametric Kruskal-Wallis test.

```{r q3_bp}
kruskal_res <- kruskal.test(price ~ aux_age, data = df); kruskal_res
```
The test returns a *p.value* of
...
```
Anova results show that the main effects of both factors are significant.
...
```
We execute Fligner-Killeen test with each factor and the interaction of both. The resulting *p.values* are
`r fligner_price$p.value` for the price, `r fligner_age$p.value` for the age and `r fligner_int$p.value`
combining both. In the case of age and age:fuelType, results show that there is clear evidence to reject the
null hypotesis of equal variances for all groups. The results when grouping by fuelType are more inconclusive
as the *p.value* is slightly over significance level.
...
null hypotesis of equal means. This implies that the new model significantly improves
on the previous one.
