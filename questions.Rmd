...
model_bc <- lm(log(price) ~ . + I(age^2) + boxCoxVariable(log(price)), data = dd)

bc <- boxcox(model_bc)
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((log(price)^lambda - 1) / lambda) ~ . + boxCoxVariable((log(price)^lambda - 1) / lambda), data = dd)
...
```{r}
# library(mvoutlier)
# df %>% select(where(is.numeric), -tax) %>% aq.plot()
...
df %>%
    rowid_to_column() %>%
    select(-starts_with("aux_"), -c(manufacturer, stud_resids, stud_outlier)) %>%
    slice(ids_combined) %>%
    arrange(rowid) %>%
    kable(
