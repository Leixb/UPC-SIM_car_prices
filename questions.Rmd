...
```{r q9_models}
model_add <- lm(price ~ ., data = select(df, where(is.numeric), -price_log))
model_add_nomil <- lm(price ~ ., data = select(df, where(is.numeric), -price_log, -mileage))
...
```{r}
model_so_far <- lm(price ~ . + I(age^2), data = select(df, where(is.numeric), -price_log))
...
log_model <- lm(price_log ~ . + I(age^2), data = select(df, where(is.numeric), -price))
...
```{r}
m5_model <- lm(price_log ~ ., data = select(df, -price, -year, -starts_with("aux_")))
m5 <- lm(price_log ~ ., data = select(df, -price, -year, -model, -starts_with("aux_")))
anova(m5, test = "Chisq")
summary(m5)
```
...
```{r}
dd <- df %>% select(-price, -year, -starts_with("aux_"))
model_bc <- lm(price_log ~ . + I(age^2) + boxCoxVariable(price_log), data = dd)

bc <- boxcox(model_bc)
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((price_log^lambda - 1) / lambda) ~ . + boxCoxVariable((price_log^lambda - 1) / lambda), data = dd)
...
```{r}
# library(mvoutlier)
# df %>% select(where(is.numeric), -price_log, -tax) %>% aq.plot()

df_outliers %>% rowid_to_column() %>% filter(moutlier) %>% pull(rowid)
```
...
```{r dfbetas_plot, fig.cap = "Influential data with DFBETAS", fig.width = 8, fig.height = 6}
influential %>%
    select(-dffit) %>%
...
```{r diffit_plot, fig.cap = "Influential data with DFFIT", fig.width = 8, fig.height = 6}
influential %>%
...
```{r}
df %>% slice(ids_dfbetas)
df %>% slice(ids_dffit)

df %>%
    rowid_to_column() %>%
    select(-starts_with("aux_"), -c(manufacturer, price_log, stud_resids, stud_outlier)) %>%
    slice(ids_combined) %>%
    arrange(rowid) %>%
    kable(
...
```{r q8_car_data}
df_mean <- df %>%
    summarise(across(c(where(is.numeric), -stud_resids, -age), mean))
...
For the data shown in \cref{tab:q8_car_data} we obtain an expected `price`
