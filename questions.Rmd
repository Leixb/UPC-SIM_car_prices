...
```{r q6_stats, fig.height = 4, fig.width=6}
lm_tibble(model) %>%
    kable(
...
```{r q6_resid, fig.height = 6, fig.width = 8}
autoplot(model1)
```

\Cref{fig:q6_stats} shows the linear regression model on `price ~ age`. The
...
is `r q6$p.value` which is less than 0.05 and thus we reject the NULL hypothesis
of homoskedasticity.
