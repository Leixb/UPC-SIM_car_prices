...
```{r outliers_ind}
soutliers %>% group_by(n_outliers) %>% summarise(count = n()) %>%
...
package. We found `r sum(df$moutlier)` multivariate outliers.
...
df[df$engineSize == "0", ] <- NA
```

```{r imputation, cache = TRUE, results='hide'}
...
write_rds_w_checksum(df_imp, "./data/dataset.rds")
```
